...
import { Users, CheckCircle, XCircle, Download, Search, RefreshCw, Key } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // Fetch RSVPs with joined invitations data
      ...
      const records = (data || []) as unknown as RSVPRecord[];
      setRsvps(records);
      ...
      records.forEach(r => {
        if (r.is_attending) {
          attendingCount++;
          // Add extra guests if they are attending too
      ...
      setStats({
        totalInvitations: records.length,
  ...
  const exportToCSV = () => {
    ...
    // CSV format creation with UTF-8 BOM so Excel opens it with accents correctly
    const csvContent = '\uFEFF' + [
      headers.join(';'),
    ...
    link.setAttribute('download', `confirmados_boda_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-[#44483f] mt-1">Control de asistencia y acompañantes en tiempo real</p>
          </div>
